...
export const DISTRICTS = [
  ...
  },
  {
    ...
    names: {
      ...
      te: "శివగంగ (శెట్టినాడ్) జిల్లా",
      hi: "शिवगंगा (चेट्टिनाड) जिला",
      ml: "ശിവഗംഗ (ചെട്ടിനാട്) ജില്ല",
      kn: "ಶಿವಗಂಗಾ (ಚೆಟ್ಟಿನಾಡ್) ಜಿಲ್ಲೆ"
    },
    zone: "South-East TN",
    zones: {
      en: "South-East TN",
...
export const EXPERIENCES = [
  ...
  { 
    ...
    tamilTitle: "மீனாட்சி அம்மன் கோயில்",
    category: "heritage", 
    districtId: "madurai", 
    districtName: "Madurai District", 
    location: "Temple Ring Road, Madurai", 
    duration: "4 Hours", 
    price: "₹200", 
    rating: 4.98, 
  ...
  { 
    ...
    tamilTitle: "திருமலை நாயக்கர் மஹால்",
    category: "heritage", 
    districtId: "madurai", 
    districtName: "Madurai District", 
  ...
  { 
    ...
    tamilTitle: "காந்தி நினைவு அருங்காட்சியகம்",
    category: "heritage", 
    districtId: "madurai", 
    districtName: "Madurai District", 
  ...
  { 
    ...
    tamilTitle: "பழமுதிர்ச்சோலை முருகன் கோயில்",
    category: "heritage", 
    districtId: "madurai", 
    districtName: "Madurai District", 
    location: "Alagar Hills Forest, Madurai", 
    duration: "2 Hours", 
    price: "Free Entry", 
    rating: 4.92, 
  ...
  { 
    ...
    tamilTitle: "வண்டியூர் மாரியம்மன் தெப்பக்குளம்",
    category: "heritage", 
    districtId: "madurai", 
    districtName: "Madurai District", 
  ...
  { 
    id: "mad-8", 
    title: "Tirupparankundram Murugan Temple", 
    tamilTitle: "திருப்பரங்குன்றம் முருகன் கோயில்",
    category: "heritage", 
    districtId: "madurai", 
    districtName: "Madurai District", 
...
/**
 * Dynamically localizes an Experience object for the active selected language
 */
export function getDistrictLabel(dist, language = "en") {
  if (!dist) return "";
  if (typeof dist === "string") {
    const found = DISTRICTS.find((d) => d.id === dist || d.name === dist);
    if (found) return getDistrictLabel(found, language);
    return dist;
  }
  if (dist.names && dist.names[language]) return dist.names[language];
  if (language === "ta" && dist.tamilName) return dist.tamilName;
  return dist.name || "";
}

export function getLocalizedExperience(exp, language = "en") {
  if (!exp) return null;

  const dist = DISTRICTS.find((d) => d.id === exp.districtId);
  const districtName = (dist && dist.names && dist.names[language])
    ? dist.names[language]
    : (dist ? (language === "ta" ? dist.tamilName : dist.name) : exp.districtName);
  ...
  const categoryLabel = (categoryLabels[exp.category] && categoryLabels[exp.category][language])
    ? categoryLabels[exp.category][language]
    : exp.category.toUpperCase();

  let title = exp.title;
  let description = exp.description;

  if (language === "ta" && exp.tamilTitle) {
    title = exp.tamilTitle;
  }
  if (language === "ta" && exp.tamilDescription) {
    description = exp.tamilDescription;
  }
